...
namespace PrimeNG.HelperFunctions {
    public static class PrimeNGHelper {
        ...
        public static (bool, byte[]?, string) GenerateExcelReport<T>(PrimeNGPostRequestWithExport inputDataAll, IQueryable<T> baseQuery, MethodInfo stringDateFormatMethod, List<string>? defaultSortColumnName = null, List<int>? defaultSortOrder = null, string sheetName = "MAIN", byte pageStack = 250) {
            try {
                PrimeNGPostRequest inputData = new PrimeNGPostRequest {
                ...
                long totalRecords = 0; // Number of records after applying filters
                string fieldName;
                GetDynamicQueryBase<T>(ref inputData, ref baseQuery, stringDateFormatMethod, ref totalRecordsNotFiltered, ref totalRecords, defaultSortColumnName, defaultSortOrder, inputDataAll.ApplySorts, inputDataAll.ApplyFilters);
